<template>
  <div class="py-5">
    <div class="flex m-1 space-x-2">
      <img class="rounded-full w-8 h-8 sm:w-10 sm:h-10 hover:cursor-pointer"
    ...
    <div class="flex-1 rounded-lg leading-relaxed m-1">
      <div class="bg-white dark:bg-ourvoice-blue px-4 py-2 rounded-lg drop-shadow-md">
        <h1 class="flex">
        ...
        <div id="fileUrlsWrapper" class="text-right">
          <a class="text-blue-400" v-for="(url, index) in presignedUrls" :key="index" :href="url" target="_blank">
            file{{ index + 1 }} 
        ...
        <div class="flex text-gray-500 dark:text-gray-300">
          <div
            ...
            <span class="hidden sm:inline-block px-1"> Comment </span>({{ post?.comments?.length ?? 0 }})
          </div>
          <div
            class="hover:text-indigo-400 dark:hover:hover:text-indigo-400 my-auto mr-1 bg-gray-100 dark:bg-gray-500 px-2 py-1 rounded-full text-xs md:text-sm hover:cursor-pointer">
      ...
      <CommentTile v-if="showComment" :postId="post.id" :noOfComments="post?.comments?.length ?? 0" />
      <div v-if="showComment" class="flex justify-center mt-5 mx-5 border-b items-center" >
        <button @click="showComment = false"><font-awesome-icon icon="fa-solid fa-chevron-up" fade /> Fold</button>
...
watchEffect(async () => {
  const res = await postsStore.getPresignedUrls('test-bucket', props.post.files ?? [], 60000) as any

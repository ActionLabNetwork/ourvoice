...
const moderations = computed(() => {
  ...
  const modifyModerations: History[] =
    version.value.version > 1
      ? [
          {
            id: version.value.id,
            decision: 'MODIFIED',
            reason: version.value?.reason,
            timestamp: version.value?.timestamp,
            moderatorHash: version.value?.authorHash,
            moderatorNickname: version.value?.authorNickname
          }
        ]
      : []

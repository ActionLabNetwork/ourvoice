<template>
  <ul class="divide-y divide-gray-100 overflow-hidden bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
    <li v-for="version in props.versions" :key="version.id" class="relative flex justify-between items-center gap-x-6 p-3 hover:bg-slate-100 hover:cursor-pointer sm:px-6" :class="isSelected(version) && 'bg-slate-200'" @click="() => handleVersionClick(version)">
      <div class="flex gap-x-4 items-center">
        ...
        <p v-if="version.moderations.length > 0 && version.moderations[0].timestamp" class="mt-1 text-xs leading-5 text-gray-500">
          Last moderated <br />
        ...
        <div>
          <span v-if="version.id === originalCommentVersion.id" class="inline-flex flex-shrink-0 rounded-full bg-green-50 px-1.5 py-0.5 text-xs font-medium text-green-700 ring-1 ring-inset ring-green-600/20">
            Original
          </span>
...
const props = defineProps({
  versions: {
    type: Array as PropType<CommentVersion[]>,
    required: true
  }
})
...
const selected = ref<CommentVersion | null>(null)
const isSelected = computed(() => {
  return (commentVersion: CommentVersion) => commentVersion === selected.value
})
...
const handleVersionClick = (version: CommentVersion) => {

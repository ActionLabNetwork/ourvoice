  ...
  <div data-cy="comment-version-list">
    ...
    >
      <li
        ...
        :class="clsx(isSelected(version) && 'bg-slate-200')"
          ...
          <div class="hidden sm:flex sm:flex-col sm:items-start space-y-2">
            ...
            <div data-cy="action-badge">
              <span
                :class="
                  clsx(
                    'inline-flex flex-shrink-0 rounded-full px-1.5 py-0.5 text-xs font-medium ring-1 ring-inset',
                    version.id === originalCommentVersion.id
...
const isSelected = computed(() => {
  return (commentVersion: ModerationCommentVersion) =>
    commentVersion === selected.value
})
